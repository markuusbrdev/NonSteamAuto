import { app, protocol, net, ipcMain, dialog, BrowserWindow } from 'electron';
...
async function saveApiKey(apiKey) {
  ...
  await fs$1.writeFile(configPath, JSON.stringify(config, null, 2));
}
...
async function getNonSteamLibrary() {
  ...
  try {
    ...
    return shortcuts.map((s) => {
      ...
      const shortcutString = `\0${s.AppName}\0${cleanExe}`;
      const appId = (CRC32.str(shortcutString) | 2147483648) >>> 0;
      return {
        appId,
        name: s.AppName,
...
async function removeNonSteamShortcut(appId) {
  ...
  if (shortcutsData.shortcuts) {
    Object.keys(shortcutsData.shortcuts).forEach((key) => {
      const s = shortcutsData.shortcuts[key];
      const shortcutString = `\0${s.AppName}\0${s.Exe}`;
