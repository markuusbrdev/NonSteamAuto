...
export async function downloadVCRedist(onProgress?: (percent: number) => void): Promise<string> {
  const url = 'https://aka.ms/vs/17/release/vc_redist.x64.exe';
  const tmpDir = os.tmpdir();
  const filePath = path.join(tmpDir, 'vc_redist.x64.exe');

  // Se já existir, assume que está baixado
  ...
  return new Promise((resolve, reject) => {
    ...
    writer.on('error', (err: any) => { // eslint-disable-line @typescript-eslint/no-explicit-any
      fs.unlink(filePath, () => {});
      reject(err);
    });
  });
...
export async function getPrefixPath(appId: string): Promise<string> {
  const paths = [
...
export async function findWineBinary(): Promise<string> {
  try {
    const { stdout } = await execAsync('which wine');
    if (stdout.trim()) return 'wine';
  } catch (err) {}

  // Fallback para procurar o wine do Proton em pastas comuns
  const searchPaths = [
...
export async function installVCRedist(appId: string, onProgress?: (percent: number) => void): Promise<boolean> {
  // 1. Verificar se Wine Nativo existe
  let wineCmd = '';
  try {
  ...
  try {
    ...
    // VCRedist pode retornar 3010 se pedir restart ou 1638 se já instalado
    // Em bash, exit code diferente de 0 joga erro no execAsync
